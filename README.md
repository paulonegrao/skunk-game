...
1. 1. SkunkGame.java: class that controls the main components/rules of the game and also handles all connections and messages between the server and its multiple clients (browsers);
1. 2. Player.java: class that represents each individual player with methods to perform the player/tokens movements in the game;
1. 3. Dice.java: class that simulates all the necessary dice for the game;
1. 4. DiceRotation: class that controls all the possible dice rotations/results;
1. 5. Score.java: class that handles the individual and total scores of the game;
1. 6. index.jsp: a unique JSP to receive the movements/instructions from the Player.java  class and render the graphic interface presentation accordingly.
